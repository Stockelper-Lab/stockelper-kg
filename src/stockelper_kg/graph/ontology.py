...
_NODE_DEFINITIONS: Tuple[NodeDefinition, ...] = (
    ...
    NodeDefinition(
        name="Event",
        description="기업의 사건(공시 카테고리 기반 이벤트; 뉴스 기반 이벤트 추출은 추후)",
        separation_reason="시점, 영향, 참여 주체가 구분되는 행위 단위",
        examples=("유상증자_결정", "전환사채권_발행결정", "자기주식_취득_결정"),
        ...
        properties=(
            ...
            NodeProperty(
                ...
                "source",
                "내부",
                '"DART" 또는 "NEWS"(NEWS는 추후)',
            ),
            NodeProperty(
                "공시 카테고리",
                "disclosure_category",
                "PostgreSQL (dart_* major-report tables)",
                'row["category"]  # 예: "증자감자", "사채발행", "자기주식", "합병분할"',
            ),
            NodeProperty(
                "공시 엔드포인트(유형)",
                "report_type",
                "PostgreSQL (dart_* major-report tables)",
                'row["report_type"]  # 예: "piicDecsn"',
            ),
            NodeProperty(
                "공시 타입 코드(백테스팅 대상 16종)",
                "disclosure_type_code",
                "내부 매핑",
                'DART_MAJOR_REPORT_TYPE_CODE_MAP[report_type]  # 예: 6,7,8,9,16,17,...',
            ),
            NodeProperty(
                "공시명(한글)",
                "disclosure_name",
                "내부 매핑 / OpenDART",
                'DART_MAJOR_REPORT_NAME_MAP[report_type]  # 예: "유상증자_결정"',
            ),
            NodeProperty(
                "발생일(접수일)",
                "rcept_dt",
                "PostgreSQL (dart_* major-report tables)",
                'row["rcept_dt"]  # DATE 또는 YYYYMMDD',
            ),
        ),
    ),
    NodeDefinition(
